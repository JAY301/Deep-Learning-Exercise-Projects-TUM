...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        ############################################################################
        H1,O1 = affine_relu_forward(X, self.params['W1'], self.params['b1'])
        scores, O2 = affine_forward(H1, self.params['W2'], self.params['b2'])
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        dx2, grads['W2'], grads['b2'] = affine_backward(dscores,O2)
        dx1, grads['W1'], grads['b1'] = affine_relu_backward(dx2,O1)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1,self.num_layers + 1):
            ...
            if i == self.num_layers:
                scores, O = affine_forward(scores,self.params[W_name],self.params[b_name])
            else:
                if self.use_batchnorm:
                    scores,self.batchnorm_O[batchnorm_name] = batchnorm_forward(scores,self.params[gamma_name],self.params[beta_name],self.bn_params[i-1])
                scores, O = affine_relu_forward(scores,self.params[W_name],self.params[b_name])
                if self.use_dropout:
                    scores,self.dropout_O[dropout_name] = dropout_forward(scores,self.dropout_param)
        ...
        for i in range(self.num_layers,0,-1):
            ...
            if i == self.num_layers:
                dscores, grads[W_name], grads[b_name] = affine_backward(dscores,self.O[O_name])
            else:
                if self.use_dropout:
                    dscores= dropout_backward(dscores,self.dropout_O[dropout_name])
                    
                dscores, grads[W_name], grads[b_name] = affine_relu_backward(dscores,self.O[O_name])
                
                if self.use_batchnorm:
                    dscores, grads[gamma_name], grads[beta_name] = batchnorm_backward(dscores, self.batchnorm_O[batchnorm_name])
